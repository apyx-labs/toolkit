...
/// [`rebase_middleware`](crate::rebase_middleware) can rewrite it.
...
///
/// Implemented for [`ClientWithMiddleware`]; authentication is provided by
/// registering [`auth_middleware`](crate::auth_middleware) on the client
/// rather than by a bespoke client type. Every request carries a static
/// [`RouteLabel`], so registering a
/// [`MetricsMiddleware`](apyx_middleware::MetricsMiddleware) yields
/// correctly-labeled, low-cardinality HTTP client metrics.
// The futures of the concrete `ClientWithMiddleware` impl are `Send`; callers
// generic over `DuneClient` that need `Send` futures should bound on the
